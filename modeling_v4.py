...
def predict_multiple_steps(X_val_data_tensor, y_val_data_tensor, num_steps, emu_len=152, chip_len=1725):
    ...
    x_input_emu = X_val_data_tensor[:n, :emu_len].float() # [1,1877] -> [1,152]
    # Get first row of 6507
    x_input_6507 = X_val_data_tensor[:n, emu_len:emu_len+chip_len].float() # [1,1877] -> [1,1725]
    x_new = np.concatenate((x_input_emu, x_input_6507), axis=1) # [1,152] & [1,1725] -> [1,1877]
    print('shape of first input',x_new.shape)
    x_new = torch.from_numpy(1*(np.array(x_new)>0.5)).float()
    x_new = x_new.to(device)

    # Generate first prediction
    y_new = y_val_data_tensor[:n,:].float()  # [1,4385]
    y_new = y_new.to(device)
    # print(len(y_new))
    ...
    for n in range(2,num_steps-1):
        ...
        # Update new input to be the prediction of the previous state
        x_new_emu = (X_val_data_tensor[ :n, :emu_len]).to(device) # [n,1877] -> [n,152]
