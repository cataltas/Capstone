#!/bin/bash
# Usage: python -u modeling.py hidden_dim bsize ep LR data_size
...
def plot_losses(train_losses, val_losses):
    ...
    plt.plot(val_losses)
    plt.title('Train & Validation Loss ({})'.format(loss_criterion))
...
if __name__ == "__main__":
    ...
    ep = sys.argv[3]
    LR = sys.argv[4]
    # whether to use full dataset or downsampling
    data_size = sys.argv[5]
    ...
    if data_size=='full':
        ...
        X_tensor, y_tensor = load_data('X_downsampled.csv', 'y_downsampled.csv')
    ...
    if data_size=='full':
        plt.savefig('nodisc_full_loss_hdim{}_bs{}_ep{}_lr{}.png'.format(hidden_dim,bsize,ep,LR))
    else:
        plt.savefig('nodisc_loss_hdim{}_bs{}_ep{}_lr{}.png'.format(hidden_dim,bsize,ep,LR))
    ...
    plt.xlabel('n')
    plt.ylabel('Loss ({})'.format(loss_criterion))
    plt.title('Loss predicting n successive steps')
    if data_size=='full':
        plt.savefig('nodisc_full_iterative_losses_hdim{}_bs{}_ep{}_lr{}.png'.format(hidden_dim, bsize, ep, LR))
    else:
        plt.savefig('nodisc_iterative_losses_hdim{}_bs{}_ep{}_lr{}.png'.format(hidden_dim, bsize, ep, LR))
